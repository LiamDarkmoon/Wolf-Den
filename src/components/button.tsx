...
export default function Button({ children, to, type, className }: { children: React.ReactNode; to?: string; type?: "button" | "submit"; className?: string }) {
    ...
    return (
        ...
        <button type={type ?? "button"} onClick={() => handleNavigate(to)}  className={`group relative flex items-center justify-center cursor-pointer ${className}`}>
            ...
            <span className="absolute text-primary group-hover:text-primary-hover group-active:text-primary group-hover:scale-105 group-active:scale-95">{children}</span>
        </button>
    );
}